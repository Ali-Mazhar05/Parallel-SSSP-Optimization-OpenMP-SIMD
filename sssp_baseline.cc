// sssp_baseline.cc  — original CilkPlus / parlay implementation (unmodified)
// Compiled by: make baseline
// Binary produced: bin/sssp_baseline

#include "sssp_baseline.h"
...
void run(Algo &algo, const Graph &G, bool verify) {
  for (int v = 0; v < NUM_SRC; v++) {
    ...
    ofstream ofs("results/baseline.tsv", ios_base::app);
    ofs << average_time << '\t';
    ofs.close();
