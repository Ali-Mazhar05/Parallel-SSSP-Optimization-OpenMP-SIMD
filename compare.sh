...
#
# Usage:
#   ./compare.sh -i <graph_file> [-a <algorithm>] [-p <param>] [-t <threads>] [-w] [-s]
#
# Options:
...
#   -t   Also include this OpenMP thread count in the sweep
#   -w   Weighted graph
#   -s   Symmetrized graph
#   -h   Show this help
#
...
# ── Parse arguments ───────────────────────────────────────────────────────────
while getopts "i:a:p:t:wsh" opt; do
  case $opt in
    i) INPUT="$OPTARG"  ;;
    a) ALGO="$OPTARG"   ;;
    p) PARAM="$OPTARG"  ;;
    t) EXTRA_THREADS="$OPTARG" ;;
    w) WEIGHTED="-w"    ;;
    s) SYMMETRIZED="-s" ;;
    h) usage            ;;
...
if [[ -n "$EXTRA_THREADS" ]]; then
  found=false
  for T in "${OMP_THREAD_COUNTS[@]}"; do
    [[ "$T" == "$EXTRA_THREADS" ]] && found=true
  done
  if ! $found; then
    OMP_THREAD_COUNTS+=("$EXTRA_THREADS")
  fi
fi
...
# ══════════════════════════════════════════════════════════════════════════════
TOTAL_STEPS=$((1 + ${#OMP_THREAD_COUNTS[@]}))
echo -e "${YEL}[1/${TOTAL_STEPS}] Running BASELINE...${RST}"
ts=$(date +%s%N)
...
for T in "${OMP_THREAD_COUNTS[@]}"; do
  ...
  echo -e "${YEL}[$((step))/${TOTAL_STEPS}] Running OPENMP with ${T} threads...${RST}"
  ts=$(date +%s%N)
...
for ((i=0; i<N; i++)); do
  ...
  for T in "${OMP_THREAD_COUNTS[@]}"; do
    read -ra OMP_TIMES <<< "${OMP_TIMES_MAP[$T]}"
    ot="${OMP_TIMES[$i]:-0}"
