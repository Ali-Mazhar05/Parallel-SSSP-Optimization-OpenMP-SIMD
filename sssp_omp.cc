// sssp_omp.cc — OpenMP-optimized implementation
// Compiled by: make omp
// Binary produced: bin/sssp_omp
//
// Optimizations applied (all in sssp_omp.h):
//   1. Degree-aware prefix-sum work partitioning
//   2. Thread-local frontier buffers + parallel prefix merge
//   3. Cache-line padding (PaddedCounter) — eliminates false sharing
//   4. Nested parallelism for high-degree vertices
//   5. SIMD (AVX2) vectorized WriteMin inner loop
...
void run(Algo &algo, const Graph &G, bool verify) {
  for (int v = 0; v < NUM_SRC; v++) {
    ...
    ofstream ofs("results/omp.tsv", ios_base::app);
    ofs << average_time << '\t';
    ofs.close();
...
int main(int argc, char *argv[]) {
  if (argc == 1) {
    fprintf(stderr,
            "Usage: %s [-i input_file] [-p parameter] [-w] [-s] [-v] [-a algorithm]\n"
            "Options:\n"
            "\t-i,\tinput file path\n"
  ...
  while ((c = getopt(argc, argv, "i:p:a:wsv")) != -1) {
    switch (c) {
      ...
      case 'p': param = string(optarg); break;
      case 'a':
        if (!strcmp(optarg, "rho-stepping"))       algo = rho_stepping;
        else if (!strcmp(optarg, "delta-stepping")) algo = delta_stepping;
        else if (!strcmp(optarg, "bellman-ford"))   algo = bellman_ford;
  ...
  if (!weighted) { printf("Generating edge weights...\n"); G.generate_weight(); }

  fprintf(stdout, "Running on %s: |V|=%zu, |E|=%zu, param=%s, num_src=%d, num_round=%d\n",
  ...
  if (algo == rho_stepping) {
    ...
    EdgeTy delta = 1 << 15;
    if (param != "") {
      if constexpr (is_integral_v<EdgeTy>) delta = stoull(param);
      else                                  delta = stod(param);
    }
